...
def H(): return {"User-Agent":random.choice(USER_AGENTS),"Accept-Language":"fa,en;q=0.8"}
def fetch(url,params=None,timeout=20): return requests.get(url,params=params,headers=H(),timeout=timeout).text
def host(u): return (urlparse(u).hostname or "").lower()

def extract_contacts(html):
    emails=re.findall(r"[A-Za-z0-9._%+\-]+@[A-Za-z0-9.\-]+\.[A-Za-z]{2,}",html)
    phones=re.findall(r"(?:\+?98|0)?9\d{9}",html)
    return {"email":emails[0] if emails else None,"phone":phones[0] if phones else None,"whatsapp":None}
...
def scrape_site(url):
    ...
    try: 
        ...
        if "<title>" in html: out["name"]=BeautifulSoup(html,"lxml").title.get_text(strip=True)[:100]
...
def startpage_search(q,want=30):
...
def multi_search(q,engines):
    ...
    for e in engines:
        try:
            if e=="startpage": pool+=startpage_search(q)
            elif e=="ddg_lite": pool+=ddg_lite_search(q)
            elif e=="mojeek": pool+=mojeek_search(q)
            elif e=="google": pool+=google_search(q)
        except: continue
    return pool
